...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, query, keys):
        ...
        context = torch.sum(attn_weights * keys, dim=1, keepdim=True)
...
def showAttention(input_sentence, output_words, attentions, index=0):
    ...
    # 手动调整子图边距（避免 tight_layout 改变宽高比）
    plt.subplots_adjust(left=0.2, right=0.9, top=0.85, bottom=0.2)

    prefix = "_".join(input_sentence.split()[:3])
    filename = f"./attention_{index}_{prefix}.png"
    plt.title(f"Attention - {input_sentence}")
...
def evaluateAndShowAttention(input_sentence, index=0):
    ...
    # 检查 attentions 是否为 None
    if attentions is not None:
        showAttention(input_sentence, output_words, attentions[0, :len(output_words), :], index)
    else:
...
for idx, sentence in enumerate(test_sentences):
    evaluateAndShowAttention(sentence, idx)